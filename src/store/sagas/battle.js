...
export function* fetchLogsSaga(action) {
  try {
    ...
    response = yield axios.get(
      "http://localhost:4001/battle/log?id=" + action.id
    );
...
export function* resetBattleSaga(action) {
  try {
    let response = yield axios.patch("http://localhost:4001/battle/reset?id=", {
      id: action.id,
    });
...
export function* newBattleSaga(action) {
  try {
    let response = yield axios.post("http://localhost:4001/battle", {
      name: action.name,
    });
...
export function* newAttackSaga(action) {
  try {
    let response = yield axios.post("http://localhost:4001/battle/start", {
      id: action.id,
    });
...
export function* addArmySaga(action) {
  try {
    let response = yield axios.post(
      "http://localhost:4001/battle/army",
      action.data
    );
    yield put(actions.addArmySuccess(response.data));
    if (action.url) {
      let res = yield axios.get(action.url);
      console.log(res);
      yield put(actions.fetchBattlesSuccess(res.data));
